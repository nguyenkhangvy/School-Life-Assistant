...
- **Tuition:** `xemhocphi` is only a form for choosing a report. The amounts are in the report "Tổng Hợp Học Phí Một Sinh Viên" (`HP1SV.001`), drawn by a Stimulsoft report viewer: the agent opens `/dg/Report/ReportDisplay?t=HP1SV.001&nhhk=<term>&masv=<own student ID>`, then asks `/dg/Report/ReportDisplayV` for the first page (`mvcviewer_parameters` = Base64 JSON, as the viewer's script sends). The page is an HTML grid; values are matched to headers by column position. The report has no due date.
